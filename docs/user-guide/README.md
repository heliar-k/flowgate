# User Guide

Welcome to the FlowGate User Guide! This section contains documentation for end users and operators.

## Contents

- **[Installation](installation.md)** - How to install and set up FlowGate
- **[Quick Start](quick-start.md)** - Get started quickly with basic usage
- **[Configuration](configuration.md)** - Configuration reference and migration guides
- **[Authentication](authentication.md)** - OAuth setup and auth artifacts
- **[CLI Reference](cli-reference.md)** - Complete command-line interface reference
- **[Troubleshooting](troubleshooting.md)** - Common issues and solutions

## Quick Links

- [Project Overview](../../README.md)
- [Developer Guide](../developer-guide/README.md)
- [Architecture Documentation](../architecture/README.md)

---

**Target Audience**: End users, operators, system administrators

**Prerequisites**: Basic command-line knowledge, familiarity with proxy servers

For development-related documentation, see the [Developer Guide](../developer-guide/README.md).
