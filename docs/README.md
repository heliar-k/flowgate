# FlowGate Documentation

Welcome to FlowGate documentation! This guide will help you get started with FlowGate, a local control tool for managing CLIProxyAPIPlus + LiteLLM stacks.

## Documentation Structure

### 📚 [User Guide](user-guide/README.md)
End-user documentation for installing, configuring, and using FlowGate.

- [Installation Guide](user-guide/installation.md)
- [Quick Start](user-guide/quick-start.md)
- [Configuration Reference](user-guide/configuration.md)
- [Profile Management](user-guide/profiles.md)
- [Authentication Setup](user-guide/authentication.md)
- [CLI Command Reference](user-guide/cli-reference.md)
- [Troubleshooting](user-guide/troubleshooting.md)

### 🛠️ [Developer Guide](developer-guide/README.md)
Documentation for contributors and developers working on FlowGate.

- [Getting Started](developer-guide/getting-started.md)
- [Project Structure](developer-guide/project-structure.md)
- [Testing Guidelines](developer-guide/testing.md)
- [Contributing Guide](developer-guide/contributing.md)
- [Release Process](developer-guide/release-process.md)
- [Path Resolution](developer-guide/path-resolution.md)

### 🏗️ [Architecture](architecture/README.md)
System architecture, design decisions, and component documentation.

- [Data Flow Diagrams](architecture/data-flows.md)
- [Diagrams](architecture/diagrams.md)
- [C4 Context](architecture/c4-context.md)
- [C4 Containers](architecture/c4-containers.md)
- [C4 Components (CLI)](architecture/c4-components-cli.md)
- [C4 Deployment](architecture/c4-deployment.md)

### 🚀 [Operations Guide](operations/README.md)
Documentation for deploying and operating FlowGate in production.

- [Deployment Guide](operations/deployment.md)
- [Monitoring Setup](operations/monitoring.md)
- [Backup & Restore](operations/backup-restore.md)
- [Upgrade Procedures](operations/upgrade-procedures.md)

### 📡 [API Documentation](api/README.md)
API reference and generated documentation.

- [CLI Reference](api/cli.md)
- [Python API](api/python-api.md)

### 📋 [Release History](releases/README.md)
Release notes and version history.

- [Release Notes Index](releases/README.md)
- [Changelog](../CHANGELOG.md)

### 📝 [Project Plans](archive/plans/README.md)
Historical project planning documents and completion reports (archived).

## Quick Start

**New Users**: Start with the [User Guide](user-guide/README.md)

**Developers**: Check out the [Developer Guide](developer-guide/README.md)

**Curious about Architecture?**: See [Architecture Overview](architecture/README.md)

## Getting Help

- **Issues?** See [Troubleshooting Guide](user-guide/troubleshooting.md)
- **Questions?** Check [CLI Reference](user-guide/cli-reference.md)
- **Contributing?** Read [Contributing Guide](developer-guide/contributing.md)

## Documentation Status

| Section | Status | Last Updated |
|---------|--------|--------------|
| User Guide | 🚧 In Progress | 2026-02-19 |
| Developer Guide | 🚧 In Progress | 2026-02-26 |
| Architecture | ✅ Complete | 2026-02-17 |
| Operations | 🚧 In Progress | 2026-02-26 |
| API Documentation | 🚧 In Progress | 2026-02-26 |

---

**Documentation Version**: v0.4.6
**Last Updated**: 2026-02-26
**Maintained By**: FlowGate Development Team
